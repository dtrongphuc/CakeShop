...
ALTER TABLE SIZEPRODUCT ADD CONSTRAINT FK_SIZEPRODUCT_PRODUCT FOREIGN KEY(IDPRODUCT) REFERENCES PRODUCT(IDPRODUCT) ON DELETE CASCADE
...
ALTER TABLE PRODUCT add constraint STATUS_PRODUCT DEFAULT 1 FOR STATUS
alter table product drop constraint STATUS_PRODUCT
ALTER TABLE PRODUCT DROP CONSTRAINT STATUS_PRODUCT
ALTER TABLE PRODUCT ADD STATUS INT 
SELECT * FROM PRODUCT
 alter table sizeproduct add CONSTRAINT [FK_SIZEPRODUCT_PRODUCT] FOREIGN KEY ([IDPRODUCT]) REFERENCES [dbo].[PRODUCT] ([IDPRODUCT]) on delete CASCADE
